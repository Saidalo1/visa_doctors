...
def create_improved_trigger(apps, schema_editor):
                ...
                IF TG_OP = 'UPDATE' AND OLD.parent_id IS NOT NULL AND 
                   (OLD.parent_id != NEW.parent_id OR NEW.parent_id IS NULL) THEN
                    ...
                    SET is_selectable = CASE
                        WHEN parent_id IS NULL AND EXISTS (
                            SELECT 1 FROM app_answeroption 
                            WHERE parent_id = id
                        ) THEN FALSE
                        ELSE TRUE
...
def remove_improved_trigger(apps, schema_editor):
    schema_editor.execute("""
        DROP TRIGGER IF EXISTS answer_option_selectability_trigger ON app_answeroption;
...
class Migration(migrations.Migration):
    dependencies = [
        ('app', '0009_answer_option_selectable'),
    ]

    operations = [
        migrations.RunPython(
            create_improved_trigger,
            remove_improved_trigger
        ),
    ]
